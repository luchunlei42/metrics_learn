...
function getConverted(column) {
	...
	for (var key in column) {
		switch (column[key].currency) {
			case "RMB": column[key].converted = column[key].amount;
				break;
...
function initStatisticPage() {
	...
	changeCurrency = function () {
		switch (user.checkedCurr) {
			...
			case "EUR":
				...
				else if (user.lastCurr == "RMB") { savings.freeMoney = (savings.freeMoney / global.eur).toFixed(3); }
				break;
			case "USD":
				if (user.lastCurr == "USD") { break; }
				else if (user.lastCurr == "EUR") { savings.freeMoney = (savings.freeMoney * global.eur / global.usd).toFixed(3); }
				else if (user.lastCurr == "RMB") { savings.freeMoney = (savings.freeMoney / global.usd).toFixed(3); }
				break;
		}
	...
	$("#RMBcurr, #eurcurr, #usdcurr").removeClass("currchecked");
	switch (user.checkedCurr) {
		case "RMB": $(".curr").html(" RMB ").data("curr", " RMB."); $(".savings-circle-currency").html(" RMBles "); $("#RMBcurr").addClass("currchecked");
			break;
